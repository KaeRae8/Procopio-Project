...
class CulturalSite: NSObject {
    ...
    init(name: String, location: GeoLocation, category: Int, url: String) {
        self.name = name
    ...
    convenience init(name: String, latitude: Double, longitude: Double, category: Int, url: String) {
        let location = GeoLocation(latitude: latitude, longitude: longitude)
        self.init(name: name, location: location, category: category, url:url)
    }
}
